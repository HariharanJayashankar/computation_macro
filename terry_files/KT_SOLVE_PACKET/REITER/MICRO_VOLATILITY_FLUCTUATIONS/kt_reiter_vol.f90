...
! includes shifts in micro level volatility over the cycle.
!
...
double precision :: gammaS = 25.0 !idio prod vol elasticity wrt agg prod
...
double precision :: kprimeSS(znum,knum),xistarSS(znum,knum),Vss(znum,knum),VaSS(znum,knum),VnaSS(znum,knum),&
	...
	distSS(znum,kdensenum),smultval
...
!!!!!Goal #0: compute new pr_mat_z array given values of amin1
call tauchen_vol(a) !note the timing here!
...
subroutine tauchen_vol(aggprod)
implicit none

!this function sets pr_mat_z as a function of agg prod, where you should note that z0 is 
!already set by an initialization calling the base_lib routine

!other declarations
integer :: zct,zprimect
double precision :: zmin,zmax,gridinc,smultval,aggprod

!determine end points of the grid (log space)
smultval = fsmult(aggprod)
...
do zct=1,znum
    ...
    pr_mat_z(zct,1) = normcdf(z0(1)+gridinc/2.0,rhoz*z0(zct),sigmaz*smultval)
    pr_mat_z(zct,znum) = 1.0 - normcdf(z0(znum)-gridinc/2.0,rhoz*z0(zct),sigmaz*smultval)
...
z0 = exp(z0)    
end subroutine tauchen_vol


end program kt_reiter_vol