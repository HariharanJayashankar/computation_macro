...
!base_lib.f90
!
!This version of the base_lib module is current as of 01/16/17. This
...
subroutine splintprime(xa,ya,y2a,n,x,yprime)
	implicit none
	
	!input/output declarations
	integer :: n
    double precision :: x,yprime,xa(n),y2a(n),ya(n)
    
    integer :: k,khi,klo
    double precision :: a,b,h,dyda,dydb,dadx,dbdx
...
1   if (khi-klo.gt.1) then
    ...
    h = xa(khi)-xa(klo)
    if (h.eq.0) write(*,*) 'bad xa input in splintprime'
    a = (xa(khi)-x)/h
    b = (x - xa(klo))/h
   
    dyda = ya(klo) + ((h**2.0)/6.0)*y2a(klo)*(3.0*a**2.0 - 1.0)
    dydb = ya(khi) + ((h**2.0)/6.0)*y2a(khi)*(3.0*b**2.0 - 1.0)
    dadx = -1.0/h;
    dbdx = 1.0/h;
    
    yprime = dyda*dadx+dydb*dbdx;

end subroutine splintprime
