...
! model, allowing for maintenance investment.
!
...
integer, parameter :: doIRF = 0 !do IRF simulation?

double precision, parameter :: maint = 0.011 !proportion of capital stock (in either direction around (1-delta)k) to which you can adjust for free
...
double precision :: k0(knum),z0(znum),a0(anum),kbar0(kbarnum),kdense0(kdensenum),pr_mat_z(znum,znum),pr_mat_a(anum,anum),&
					...
					MICROsim(nummicro,numper),kfcsterror,pfcsterror,kprimemaint(znum,anum,knum,kbarnum),&
					kprimemaintold(znum,anum,knum,kbarnum),kprimemaintp(znum,kdensenum)
...
double precision :: start,finish,vferror,zval,kval,kbarval,kbarfcstval,kprimeval,&
    ...
    fa,fb,fc,pvala,pvalb,pvalc,ival,padjust,kmaintlb,kmaintub,kprimemainterror,ivalnoadj
...
open(8,file="kprimemaintold.txt")
do zct=1,znum
do act=1,anum
do kct=1,knum
do kbarct=1,knum
read(8,*) kprimemaintold(zct,act,kct,kbarct)
end do
end do
end do
end do
...
do kbarct=1,kbarnum
	...
	kprimemaintold(zct,act,kct,kbarct) = max((1.0-delta)*k0(kct),kmin*1.01)
...
do vfct=1,maxvfit
    ...
    do accelit=1,maxaccelit
        ...
        do kbarct=1,kbarnum
            ...
            kprimeval = kprimemaintold(zct,act,kct,kbarct)
            Vnaval = pval * freduced(zval,kval)           
            Vnaval = Vnaval - pval * (kprimeval - (1.0-delta)*kval)
    ...
    !$omp& kbarfcstval,Vnaval,kprimeval,statect,&
    !$omp& zprimect,aprimect,stateprimect,Vnextval,Vaval,&
    !$omp& xival,Vval,Vnextval1,Vnextval2,kmaintlb,kmaintub)
    !$omp do collapse(4)
    do zct=1,znum
	do act=1,anum
	do kct=1,knum
	do kbarct=1,kbarnum
        ...
        RHSzct = zct
        RHSact = act
        kmaintlb = ( -1.0*maint + (1.0-delta) ) * kval; kmaintlb = max(kmaintlb,kmin)
        kmaintub = ( maint + (1.0-delta) ) * kval; kmaintub = min(kmaintub,kmax)
        
        Vnaval = brent(kmaintlb,0.5*(kmaintlb+kmaintub),kmaintub,fnRHS,brenttol,kprimeval) !note this sets correct kprimeval, not correct vnaval
        Vnaval = -1.0 * Vnaval
      
        !at this point, Vnaval = -p k' + beta E V'
        !now, construct current period return remaining portions
        Vnaval = Vnaval + pval * (freduced(zval,kval)+(1.0-delta)*kval)
        
        Vna(zct,act,kct,kbarct) = Vnaval
        kprimemaint(zct,act,kct,kbarct) = kprimeval
        !!end of block computuing Vnaval
    ...
    kprimemainterror = maxval(abs(log(kprimemaint)-log(kprimemaintold)))
    ...
    write(13,*) "VF iter = ",vfct,"Kprimemaint error = ",kprimemainterror
    ...
    write(*,*) "VF iter = ",vfct,"Kprimemaint error = ",kprimemainterror
    ...
    if (((kprimeerror<kprimeerrortol) .and. (kprimemainterror<kprimeerrortol) ).and.(xistarerror< xistarerrortol) ) exit
    Vold=V
    kprimeold = kprime
    xistarold = xistar
    kprimemaintold = kprimemaint
...
if (minval(kprimemaint)<1.05*k0(1)) then
    write(13,*) "Lowest idio maint capital close to bottom of grid."
    write(*,*) "Lowest idio maint capital close to bottom of grid."
end if
if (maxval(kprimemaint)>0.95*k0(knum)) then
    write(13,*) "Highest idio maint capital close to top of grid."
    write(*,*) "Highest idio maint capital close to top of grid."
end if
...
open(8,file="kprimemaint.txt")
do zct=1,znum; do act=1,anum; do kct=1,knum; do kbarct=1,kbarnum
write(8,*) kprimemaint(zct,act,kct,kbarct)
end do; end do;end do; end do
close(8)
...
!do t=1,1
    ...
    !$omp& aprimect,zprimect,Vnextval1,Vnextval2,Vnextval,Vaval,xival,Vval,kmaintlb,&
    !$omp& kmaintub,kprimevalnoadj)
    !$omp do collapse(2)
    do zct=1,znum
	do kct=1,kdensenum
        ...
        Vnaval = brent(kmaintlb,0.5*(kmaintlb+kmaintub),kmaintub,fnRHS,brenttol,kprimevalnoadj) !note this sets correct kprimeval, not correct vaval
        Vnaval = -1.0 * Vnaval
        !at this point, Vaval = -p k' + beta E V'
        !now, construct current period return remaining portions
        Vnaval = Vnaval + pval * (freduced(zval,kval)+(1.0-delta)*kval)
        
        kprimemaintp(zct,kct) = kprimevalnoadj
    ...
    !!$omp& yval,kprimevalnoadj) reduction(+:Yactualp,Iactualp,Kprimeactualp,Nactualp)
    !!$omp do collapse(2)
    do zct=1,znum
    do kct=1,kdensenum
         if (distkz(zct,kct,t)>0.0) then
        ...
        kprimevalnoadj = kprimemaintp(zct,kct)
        ...
        Iactualp = Iactualp + distkz(zct,kct,t) * cdfxi(xival) &
            * (kprimeval - (1.0-delta)*kval) + distkz(zct,kct,t) * &
            (1.0 - cdfxi(xival)) * (kprimevalnoadj - (1.0-delta)*kval)
    
        Kprimeactualp = Kprimeactualp + distkz(zct,kct,t) * cdfxi(xival) * kprimeval &
            +  distkz(zct,kct,t) * (1.0 - cdfxi(xival)) * kprimevalnoadj
 ...
 do zct=1,znum
    do kct=1,kdensenum
        if (distkz(zct,kct,t)>0.0) then
            ...
            kprimevalnoadj = kprimemaintp(zct,kct)
            ivalnoadj = kprimevalnoadj - (1.0-delta)*kval 
            xival = xistarp(zct,kct)
            padjust = cdfxi(xival)
            
            !investment rate
            MICROsim(1,t) = MICROsim(1,t) + distkz(zct,kct,t) * padjust * (ival / kval) + &
            	 distkz(zct,kct,t) * (1.0-padjust) * (ivalnoadj / kval)
            
            !investment rate squared - for stdev construction
            MICROsim(2,t) = MICROsim(2,t) + distkz(zct,kct,t) * padjust * ( (ival / kval)**2.0 ) + &
            	distkz(zct,kct,t) * (1.0 - padjust) * ( (ivalnoadj / kval)**2.0 )
            
            !P(inaction) 
             if (abs(ivalnoadj/kval)<0.01) then
	             MICROsim(3,t) = MICROsim(3,t) + distkz(zct,kct,t) * (1.0 - padjust)      	
             end if
			if (abs(ival/kval)<0.01) then
	             MICROsim(3,t) = MICROsim(3,t) + distkz(zct,kct,t) * (padjust)      	
             end if
			...
			if ((ivalnoadj/kval)>0.0) then
            	MICROsim(6,t) = MICROsim(6,t) + distkz(zct,kct,t) * (1.0-padjust)
            end if
			...
			if ((ivalnoadj/kval)<0.0) then
            	MICROsim(7,t) = MICROsim(7,t) + distkz(zct,kct,t) * (1.0 - padjust)
            end if
...
do kct=1,knum
do kbarct=1,knum
write(8,*) kprimemaintold(zct,act,kct,kbarct)
end do
end do
end do
end do
...
do t=1,numperIRF-1
    ...
    do kct=1,kdensenum
        ...
        Iactualp = Iactualp + distIRF(zct,kct,t,simct,shockct) * cdfxi(xival) &
            * (kprimeval - (1.0-delta)*kval) + distIRF(zct,kct,t,simct,shockct) * &
            (1.0 - cdfxi(xival)) * (kprimevalnoadj - (1.0-delta)*kval)
		...
		Kprimeactualp = Kprimeactualp + distIRF(zct,kct,t,simct,shockct)  * cdfxi(xival) * kprimeval &
            + distIRF(zct,kct,t,simct,shockct) * (1.0 - cdfxi(xival)) * kprimevalnoadj            
