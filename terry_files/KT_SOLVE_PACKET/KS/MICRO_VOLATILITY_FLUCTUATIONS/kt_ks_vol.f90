...
! model, allowing for micro volatility fluctuations.
!
...
double precision :: gammaS = 33.0 !idio prod vol elasticity wrt agg prod
!log(z') = rho log(z) + sigma_z * s'(a') * eps', eps' ~ N(0,1)
!log(s) = -gammaS * log(a)
...
!integer, parameter :: shockanumIRF = 1 !grid point that you impose in IRF period
...
!double precision, parameter :: shocksizeIRF=-1.0*sigmaa !shock size in IRF simulations
...
double precision :: k0(knum),z0(znum),a0(anum),kbar0(kbarnum),kdense0(kdensenum),pr_mat_z(znum,znum),pr_mat_a(anum,anum),&
					...
					MICROsim(nummicro,numper),kfcsterror,pfcsterror,pr_mat_z_BIG(znum,znum,anum)
    ...
    do kct=1,kdensenum
        ...
        do zprimect=1,znum
            ...
            distkz(zprimect,kprimeind,t+1) = distkz(zprimect,kprimeind,t+1) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * padjustp(zct,kct) * (1.0 - weight) * distkz(zct,kct,t)
            
            !transfer the weight to kprimeind+1 that does adjust
            distkz(zprimect,kprimeind+1,t+1) = distkz(zprimect,kprimeind+1,t+1) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * padjustp(zct,kct) * weight * distkz(zct,kct,t)
            ...
            distkz(zprimect,kprimeindnoadj,t+1) = distkz(zprimect,kprimeindnoadj,t+1) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * (1.0 - padjustp(zct,kct)) * (1.0 - weightnoadj) * distkz(zct,kct,t)
            ...
            distkz(zprimect,kprimeindnoadj+1,t+1) = distkz(zprimect,kprimeindnoadj+1,t+1) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * (1.0 - padjustp(zct,kct)) * weightnoadj * distkz(zct,kct,t)
...
do t=1,numperIRF-1
   ...
   if ((piter>3.and.piter<pcutoff).or.(piter>pcutoff+2)) then 
    ...
    !now that market clearing has occured, insert distribution into next period's
    aprimect=asimposIRF(t+1,simct,shockct)
    do zct=1,znum
    do kct=1,kdensenum
        if (distIRF(zct,kct,t,simct,shockct)>0.0) then
        zval = z0(zct);
        ...
        do zprimect=1,znum
            ...
            distIRF(zprimect,kprimeind,t+1,simct,shockct) = distIRF(zprimect,kprimeind,t+1,simct,shockct) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * padjustp(zct,kct) * (1.0 - weight) * distIRF(zct,kct,t,simct,shockct)
            
            !transfer the weight to kprimeind+1 that does adjust
            distIRF(zprimect,kprimeind+1,t+1,simct,shockct) = distIRF(zprimect,kprimeind+1,t+1,simct,shockct) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) * padjustp(zct,kct) * weight * distIRF(zct,kct,t,simct,shockct)
    
            !transfer the weight to kprimeindnoadj that doesn't adjust
            distIRF(zprimect,kprimeindnoadj,t+1,simct,shockct) = distIRF(zprimect,kprimeindnoadj,t+1,simct,shockct) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) &
                * (1.0 - padjustp(zct,kct)) * (1.0 - weightnoadj) * distIRF(zct,kct,t,simct,shockct)
            
            !transfer the weight to kprimeindnoadj+1 that doesn't adjust
            distIRF(zprimect,kprimeindnoadj+1,t+1,simct,shockct) = distIRF(zprimect,kprimeindnoadj+1,t+1,simct,shockct) &
                + pr_mat_z_BIG(zct,zprimect,aprimect) &
                * (1.0 - padjustp(zct,kct)) * weightnoadj * distIRF(zct,kct,t,simct,shockct)
...
double precision :: asimgrid(anum),gridinc,smultval
...
!create pr_mat_z_BIG
!P(z'|z,a'), znum,znum,anum
z0 = log(z0)
pr_mat_z_BIG(:,:,:) = 0.0

do aprimect=1,anum

	smultval = fsmult(a0(aprimect))
	gridinc = z0(2)-z0(1)
		...
		do zprimect=2,(znum-1)
			pr_mat_z_BIG(zct,zprimect,aprimect) = (normcdf(z0(zprimect)+gridinc/2.0,rhoz*z0(zct),sigmaz*smultval) - &
				normcdf(z0(zprimect)-gridinc/2.0,rhoz*z0(zct),sigmaz*smultval))
		end do !zct

		!first interval and last interval take the rest of the weight
		pr_mat_z_BIG(zct,1,aprimect) =	normcdf(z0(1)+gridinc/2.0,rhoz*z0(zct),sigmaz*smultval) 
		pr_mat_z_BIG(zct,znum,aprimect) = (1.0 - normcdf(z0(znum)-gridinc/2.0,rhoz*z0(zct),sigmaz*smultval))
...
do aprimect=1,anum
do zct=1,znum
    pr_mat_z_BIG(zct,:,aprimect) = pr_mat_z_BIG(zct,:,aprimect)/sum(pr_mat_z_BIG(zct,:,aprimect))
end do !zct
end do !act
...
do act=1,anum
    ...
    do aprimect=1,anum
		...
		do zprimect=2,(znum-1)
			stateprimect = (aprimect-1)*znum + zprimect
			pr_mat(statect,stateprimect) = pr_mat_a(act,aprimect)*( &
				normcdf(z0(zprimect)+gridinc/2.0,rhoz*z0(zct),sigmaz*smultval) - &
				normcdf(z0(zprimect)-gridinc/2.0,rhoz*z0(zct),sigmaz*smultval))
		end do !zprimect

		!first interval and last interval take the rest of the weight
		stateprimect = (aprimect-1)*znum + 1
		pr_mat(statect,stateprimect) =  pr_mat_a(act,aprimect) * &
			normcdf(z0(1)+gridinc/2.0,rhoz*z0(zct),sigmaz*smultval) 
			
		stateprimect = (aprimect-1)*znum + znum	
		pr_mat(statect,stateprimect) =  pr_mat_a(act,aprimect) * &
			(1.0 - normcdf(z0(znum)-gridinc/2.0,rhoz*z0(zct),sigmaz*smultval))
...
open(8,file="pr_mat_z_BIG.txt")
do aprimect=1,anum
do zct=1,znum
write(8,*) pr_mat_z_BIG(zct,:,aprimect)
end do !zct
end do !aprimect
close(8)
...
double precision function fsmult(aggprod)
implicit none

!this function computes idio prod shock vol multiplier as function of agg state today

double precision :: aggprod

fsmult = -1.0 * gammaS * log(aggprod)
fsmult = exp(fsmult)

end function fsmult

end program kt_ks_vol