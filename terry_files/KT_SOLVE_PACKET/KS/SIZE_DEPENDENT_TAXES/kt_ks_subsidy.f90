...
! model, extended to incorporate size-dependent taxes and subsidies.
!
...
double precision :: pval,wval,aval,weight,tau
integer :: kbarfcstind,RHSact,RHSzct
!$omp threadprivate(pval,wval,aval,kbarfcstind,weight,RHSact,RHSzct,tau)
    
end module modparams

program kt_ks_subsidy
use omp_lib
use base_lib
use modparams
implicit none
...
open(13,file="kt_ks_subsidy.txt")
...
!do t=1,1
    ...
    if (mod(t,100)==1) then
        ...
        write(*,"(A,I5,A,I5,A,F7.5,A,F7.5,A,F7.5,A,F7.5)") "t= ",t,", pit = ",piter,", err = ",perror,", p = ",pval,", aval = ",aval
    end if 
...
do t=1,numperIRF-1
    ...
    aval = a0(act); avalstore = aval; tau = ftau(aval)
...
if (k>=kappastar) then
	freduced = ( nu ** (nu * exponentnu) ) * (1.0 - nu) * ( (wval*(1.0-tau))  ** ( -1.0 * nu * exponentnu ) )
...
if (k>=kappastar) then
	nreduced = ( nu **  exponentnu ) * ( (wval*(1.0-tau)) ** ( -1.0 * exponentnu ) )
...
if (k>=kappastar) then
	yreduced = ( (nu/(wval*(1.0-tau))) ** (nu * exponentnu) ) * ( (aval * z ) ** exponentnu  )
